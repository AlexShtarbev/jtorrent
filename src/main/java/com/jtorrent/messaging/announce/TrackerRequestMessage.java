...
/**
 * An abstract class that defines the format of the request message to the
 * tracker.
 * 
 * @see <a href=
 *      "https://wiki.theory.org/BitTorrentSpecification#Tracker_Request_Parameters">Tracker
...
public abstract class TrackerRequestMessage {
	 ...
	 * first four bytes are the host (in network byte order), the last two bytes
	 ...
	 * refuse requests without "compact=1" or simply send a compact response
	 * unless the request contains "compact=0" (in which case they will refuse
	 * the request.)
	 */
	...
	public TrackerRequestMessage(byte[] infoHash, String ip, int port, String peerID, long uploaded, long downloaded,
			long left, int compact, int noPeerID, TrackerRequestEvent event, int numWant, int key) {
		_infoHash = infoHash;
		_ip = ip;
		_port = port;
		_peerID = peerID;
