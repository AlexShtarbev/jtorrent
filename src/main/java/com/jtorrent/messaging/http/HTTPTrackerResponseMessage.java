...
public class HTTPTrackerResponseMessage extends TrackerResponseMessage {
	...
	public static HTTPTrackerResponseMessage parse(ByteBuffer message) throws IOException, AnnounceException {
		...
		return new HTTPTrackerResponseMessage(params.getOrDefault(FAILURE_REASON_KEY, new BObject("")).asString(),
				params.getOrDefault(WARNING_MESSAGE_KEY, new BObject("")).asString(), params.get(INTERVAL_KEY).asInt(), // mandatory
				params.getOrDefault(COMPLETE_KEY, new BObject(0)).asInt(),
				params.getOrDefault(INCOMPLETE_KEY, new BObject(0)).asInt(), providePeerList(params));
	}

	private static List<Peer> providePeerList(Map<String, BObject> params)
			throws BEncodingException, UnsupportedEncodingException, UnknownHostException {
	...
	/**
	 * The torrent client asks for a compact list, therefore the tracker will
	 * send a string. The string consists of multiples of 6 bytes. First 4 bytes
	 * are the IP address and last 2 bytes are the port number. All the
	 * information is in big endian notation.
	 * 
	 * @param data
	 * @return
	 ...
	 */
	private static List<Peer> providePeerListFromBytes(byte[] peers)
			throws BEncodingException, UnsupportedEncodingException, UnknownHostException {
		...
		for (int i = 0; i < peers.length / 6; i++) {
			...
			int port = (0xFF & (int) peersBuffer.get()) << 8 | (0xFF & (int) peersBuffer.get());
	...
	/**
	 * The values is a list of dictionaries(maps) where each dictionary contains
	 * the following keys:
	 * <ul>
	 * <li>peer id: peer's self-selected ID, as described above for the tracker
	 * request (string)</li>
	 * <li>ip: peer's IP address either IPv6 (hexed) or IPv4 (dotted quad) or
	 * DNS name (string)</li>
	...
	public static List<Peer> providePeerListFromList(List<BObject> peers) throws BEncodingException {
		...
		for (BObject peer : peers) {
			...
			String host = peerData.get(TrackerRequestMessage.IP_KEY).asString(TorrentSession.BYTE_ENCODING);
