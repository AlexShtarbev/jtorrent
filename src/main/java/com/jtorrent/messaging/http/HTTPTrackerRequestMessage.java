...
public class HTTPTrackerRequestMessage extends TrackerRequestMessage {
	...
	public HTTPTrackerRequestMessage(byte[] infoHash, String ip, int port, String peerID, long uploaded,
			long downloaded, long left, int compact, int noPeerID, TrackerRequestEvent event, int numWant, int key) {
	...
	public URL formTrackerRequest(URL trackerURL) throws UnsupportedEncodingException, MalformedURLException {
		...
		url.append(INFO_HASH_KEY + "=").append(URLEncoder
				.encode(new String(this.getInfoHash(), TorrentSession.BYTE_ENCODING), TorrentSession.BYTE_ENCODING));
		url.append("&" + PEER_ID_KEY + "=").append(URLEncoder.encode(getPeerID(), TorrentSession.BYTE_ENCODING));
